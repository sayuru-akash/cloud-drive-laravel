...
<template>
    ...
    <div class="space-y-6">
        ...
        <div
            v-if="flash?.shareUrl"
            class="cloud-panel flex flex-col gap-3 p-4 text-sm text-ink-700 md:flex-row md:items-center md:justify-between dark:text-ink-300"
        >
            <a
                class="min-w-0 truncate font-medium text-brand"
                :href="flash.shareUrl"
                target="_blank"
                rel="noreferrer"
                >{{ flash.shareUrl }}</a
        ...
        <section class="cloud-panel divide-y divide-line p-5">
            ...
            >
                ...
                <button
                    type="button"
                    class="text-sm font-medium text-red-600 disabled:opacity-45"
                    :disabled="share.is_revoked"
                    @click="router.patch(`/shares/${share.id}/revoke`)"
                >
                    Revoke
                </button>
            </div>
