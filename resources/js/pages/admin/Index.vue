...
<template>
    ...
    <div class="space-y-6">
        <PageHeader
            title="Admin"
            description="Manage users and workspace policies."
        />
        <form
            class="cloud-panel grid gap-4 p-5 md:grid-cols-4"
            @submit.prevent="form.patch('/admin/settings')"
        >
            <label class="text-sm font-medium"
                >Upload limit<input
                    v-model.number="form.max_upload_size_bytes"
                    type="number"
                    min="1"
            ...
            <label class="text-sm font-medium"
                >Share expiry<input
                    v-model.number="form.share_expiry_days"
                    type="number"
                    min="1"
                    max="90"
                    class="mt-2 w-full rounded-full border border-line bg-white px-4 py-2 dark:bg-white/10"
            /></label>
            <label class="text-sm font-medium"
                >Blocked extensions<input
                    v-model="form.blocked_extensions"
                    class="mt-2 w-full rounded-full border border-line bg-white px-4 py-2 dark:bg-white/10"
            /></label>
            <button
                type="submit"
                class="cloud-button bg-ink-950 text-white md:col-span-4 dark:bg-white dark:text-ink-950"
            >
                Save settings
            </button>
        </form>
        <section class="cloud-panel divide-y divide-line p-5">
            ...
            >
                ...
                <select
                    class="rounded-full border border-line bg-white px-3 py-2 text-sm dark:bg-white/10"
                    :value="user.role"
                    @change="
